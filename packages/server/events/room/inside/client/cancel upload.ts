...
export default {
    name: 'cancel upload',
    listener: async (io: Server, socket: Socket, uploadPath: string, hostUploadPath: string, shouldEmitBack: boolean) => {
        try {
            ...
            */
            ...
            socket.to(host.socketId).emit('cancel upload', socket.data.user.id, uploadPath, hostUploadPath, shouldEmitBack);
        } catch (error) {
            console.log(error);
        }
    }
